...
class WhatsAppBotGPT:
    ...
    def run(self):
        ...
        while len(self.driver.find_elements(By.ID, 'pane-side')) < 1: # Aguarda até a página ser carregada completamente
            time.sleep(1)

        while self.driver.find_element(By.XPATH, r"//*[@id='app']/div/span[2]").text != "": # Esperando as mensagens serem Sincronizadas
            time.sleep(1)

        print("Mensagens Sincronizadas")

        mensagens = self.driver.find_elements(By.CSS_SELECTOR, r"#main .message-in .selectable-text")

        last_msg = mensagens[-1].text
